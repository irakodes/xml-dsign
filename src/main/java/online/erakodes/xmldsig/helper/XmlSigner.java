...
import static online.erakodes.xmldsig.helper.AppURIDereferencer.setRightContent;
import static online.erakodes.xmldsig.util.KeyHandler.getCertificate;
import static online.erakodes.xmldsig.util.KeyHandler.getPrivateKey;
import static online.erakodes.xmldsig.util.XmlConverter.convert;

public class XmlSigner {
    private static final Logger log = LoggerFactory.getLogger(XmlSigner.class);

    /**
     * Signs an XML document provided as a string using a private key and certificate
     * obtained from the provided key password. The signing process involves creating
     * a digital signature compliant with XML Signature (XMLDSIG) standards.
     * 
     * @param xmlStr the XML document to be signed, provided as a string
     * @param keyPass the password used to retrieve the private key and certificate
     * @return the signed XML document as a string
     * @throws Exception if the input document is null, the "AppHdr" element is missing,
     *                   required cryptographic operations fail, or any other signing error occurs
     */
